...
GET_BULLETIN_FRAME = bytes.fromhex("01 04 00 04 11 12 13 14")


def test_valid_header_and_operation_payload_dispatch() -> None:
    assert decode_frame(GET_BULLETIN_FRAME) == GetBulletin(286397204)


def test_encode_frame_reproduces_canonical_get_bulletin_vector() -> None:
    assert encode_frame(GetBulletin(286397204)) == GET_BULLETIN_FRAME
...
def test_unsolicited_flag_round_trips_only_through_server_frame_decoder() -> None:
    message = Message(1, 3, "EA3AAA", "EA3BBB", "proactive")
...
def test_unsolicited_flag_is_rejected_for_ineligible_operation() -> None:
    with pytest.raises(InvalidFieldError, match="only for"):
        decode_frame_with_flags(bytes.fromhex("01 04 01 08 11 12 13 14 15 16 17 18"))
...
def test_maximum_payload_reaches_operation_dispatch(
    ...
    assert decode_frame(
        bytes((1, Operation.GET_BULLETIN, 0, 255)) + payload
    ) == GetBulletin(1)
    assert seen == [payload]
...
def test_decode_accepts_bytes_only() -> None:
    with pytest.raises(ProtocolDecodeError, match="must be bytes"):
        decode_frame(bytearray(GET_BULLETIN_FRAME))
