...
def test_empty_mailbox_sync_preserves_completed_cursor(tmp_path) -> None:
    ...
    assert (
        scenario.completed_cursor(result.initial_empty_sync, Operation.GET_NEW_MESSAGES)
        == 0
    )
    assert result.synced_send == [Stored()]
    assert len(result.initial_sync) == 2
    assert isinstance(result.initial_sync[0], Message)
    assert result.initial_sync[0].body == scenario.SYNCED_MESSAGE.body
    ...
    assert result.other_send == [Stored()]
