...
def test_two_users_exchange_one_private_message_through_real_stack(tmp_path) -> None:
    ...
    assert result.recipient_mailbox == [
        Message(
            1, scenario.CREATED_AT, scenario.SENDER, scenario.RECIPIENT, scenario.BODY
        ),
        End(Operation.GET_NEW_MESSAGES, 1, 1, False),
    ]
    assert result.sender_mailbox == [End(Operation.GET_NEW_MESSAGES, 0, 0, False)]
    assert result.third_user_mailbox == [End(Operation.GET_NEW_MESSAGES, 0, 0, False)]
...
def test_scenario_accepts_a_storage_free_fake_environment() -> None:
    ...
    class FakeClient:
        ...
        def request(self, request):
            if self.callsign == scenario.SENDER and (
                not isinstance(request, scenario.GetNewMessages)
            ):
                return [Stored()]
