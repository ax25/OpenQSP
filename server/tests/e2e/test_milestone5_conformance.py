...
RECIPIENT = "EA3BBB"
UNRELATED = "EA3CCC"
MESSAGE_A = SendMessage(1786910001, RECIPIENT, "M5.5 message A")
MESSAGE_B = SendMessage(1786910002, RECIPIENT, "M5.5 message B")
MESSAGE_C = SendMessage(1786910003, RECIPIENT, "M5.5 after restart")
BULLETIN = Bulletin(
    1297416449, 1786910004, "EA9SRC", "M5.5 node bulletin", "Persistent bulletin body"
)
LIMIT = 20
...
def _assert_message(response, request: SendMessage, sequence: int) -> None:
    assert response == Message(
        sequence, request.created_at, SENDER, RECIPIENT, request.body
    )
...
def test_milestone5_complete_internet_transport_workflow(tmp_path) -> None:
    ...
    try:
        ...
        assert recipient.request(GetBulletin(1297481727)) == [
        ...
        assert sender.request(MESSAGE_C) == [Stored()]
        post_restart = recipient.request(GetNewMessages(pre_restart_cursor, LIMIT))
