...
def test_bulletin_sync_retrieval_and_resume(tmp_path) -> None:
    ...
    assert first_headers == [_header(1, scenario.BULLETIN_A), _header(2, scenario.BULLETIN_B)]
    ...
    assert first_end == End(Operation.GET_NEW_BULLETINS, 2, result.initial_cursor, False)
    ...
    assert scenario.completed_cursor(
        result.empty_sync, Operation.GET_NEW_BULLETINS
