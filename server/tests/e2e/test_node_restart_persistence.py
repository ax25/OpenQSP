...
def test_private_mailbox_sync_recovers_across_node_restart(tmp_path) -> None:
    result = scenario.run_scenario(
        LocalScenarioEnvironment(tmp_path / "persistent-node.db")
    )
    ...
    assert len({message_a.sequence, message_b.sequence}) == 2
    assert scenario.MESSAGE_A.body not in {
        item.body for item in result.incremental_sync if isinstance(item, Message)
    }
