...
import pytest
from openqsp.protocol import End, Message, Operation, SendMessage, Stored, encode_frame
...
def test_local_cli_workflow_persists_across_invocations(tmp_path) -> None:
    ...
    assert "sequence: 1" in received[1] and "author: K1ABC" in received[1]
    assert "message_id" not in received[1] and "next_since: 1" in received[1]
...
def test_development_seed_then_public_bulletin_operations(tmp_path) -> None:
    ...
    assert seeded[0] == 0 and "sequence: 1" in seeded[1]
