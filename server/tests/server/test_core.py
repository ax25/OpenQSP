...
@pytest.mark.parametrize(("protocol_request", "operation"), REQUESTS_WITHOUT_STORES)
def test_bulletin_requests_without_store_return_busy(protocol_request, operation):
...
@pytest.mark.parametrize(
    ...
    (
        (b"\x01\x02\x00\t\x00", Operation.GET_NEW_MESSAGES, ErrorCode.INVALID_FRAME),
...
def test_malformed_frame_does_not_touch_injected_stores():
    ...
    assert core.handle_frame("K1ABC", b"\x01\x02\x00\t")
