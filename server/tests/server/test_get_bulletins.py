...
def test_headers_preserve_all_stored_fields_and_order_without_bodies(tmp_path):
    ...
    assert responses[-1] == End(Operation.GET_NEW_BULLETINS, 3, sequences[-1], False)


def test_header_pagination_then_empty_page_has_no_duplicates(tmp_path):
    store = BulletinStore(_database(tmp_path))
    sequences = [_store_bulletin(store, number) for number in (1, 2, 3)]
...
def test_complete_bulletin_preserves_every_stored_field_and_is_public(tmp_path):
    ...
    persisted = store.get_bulletin(sequence=1)
    expected = Bulletin(
        persisted.sequence,
    ...
    core = ServerCore(bulletin_store=store)
    assert _bulletin_response(core, 1, callsign="EA3GNU") == [expected]
    assert _bulletin_response(core, 1, callsign="K1ABC") == [expected]
    assert (
        _new_responses(core, callsign="EA3GNU")[:-1]
        == _new_responses(core, callsign="K1ABC")[:-1]
    )


def test_missing_and_private_numbers_are_bulletin_not_found(tmp_path):
    database = _database(tmp_path)
    MessageStore(database).store_message(
        created_at=100, author="K1ABC", recipient="EA3GNU", body="private"
    )
...
def test_missing_store_returns_busy_for_both_operations():
    assert _new_responses(ServerCore()) == [
        Error(Operation.GET_NEW_BULLETINS, ErrorCode.BUSY, "bulletin store unavailable")
    ]
...
def test_bulletin_retrieval_survives_database_restart(tmp_path):
    ...
    assert _bulletin_response(core, 1) == [
        Bulletin(1, 1015, "EA9SRC", "title 15 — exact", "body 15 — not a header")
    ]
...
def test_all_milestone_three_operations_share_one_persistent_node(tmp_path):
    ...
    messages = [
        decode_frame(frame)
        for frame in core.handle_frame("EA3GNU", encode_frame(GetNewMessages(0, 20)))
    ]
    assert send == [Stored()]
    assert messages == [
