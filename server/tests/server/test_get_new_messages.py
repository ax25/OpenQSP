...
def _store_message(store, number, recipient, *, author="EA9SRC"):
    outcome = store.store_message(
        created_at=1000 + number,
        author=author,
        recipient=recipient,
        body=f"body {number} — exact",
    )
...
def test_single_message_maps_every_persisted_field_then_end(tmp_path):
    store = MessageStore(_database(tmp_path), clock=lambda: 2000)
...
def test_multiple_messages_preserve_ascending_storage_order(tmp_path):
    ...
    sequences = [_store_message(store, number, "EA3GNU") for number in (1, 2, 3)]
    responses = _responses(ServerCore(message_store=store))
...
def test_retrieval_survives_database_restart(tmp_path):
    ...
    sequences = [_store_message(store, number, "EA3GNU") for number in (1, 2)]
