...
- server-assigned mailbox `sequence`;
- `from`;
- `to`;
- `created_at`;
- `body`.
...
- no client-generated identifier;
- `recipient`;
- `created_at`;
- `body`.
...
A successful response is zero-payload `STORED`, meaning the transaction committed durably. Obsolete ID-based statuses are not part of Core:

- `STORED`: the node durably stored the message;

Failures use the normal `ERROR` operation. Transport retry and duplicate suppression are outside Core.
