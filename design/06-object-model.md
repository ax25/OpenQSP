...
- `sequence`: unsigned 32-bit sequence scoped to the object stream;
- `type`: object type;
...
## Identity scope and transport boundary

Private-message sequences are allocated independently per recipient mailbox.
Bulletin sequences are node-local and are the sole bulletin reference. Neither
object carries a retry identifier. Unreliable transport adapters may wrap Core
frames in peer-scoped transaction identifiers without adding those identifiers
