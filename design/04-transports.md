...
These mechanisms are separate from OpenQSP `STORED`, which confirms an application-level durable processing result.
...
This ACK is an **APRS transport-level acknowledgement**. It confirms reception of the APRS message packet and remains distinct from OpenQSP application-level acknowledgements such as OpenQSP `STORED`.
...
TCP/WebSocket carry Core frames over reliable ordered transport and require no
Core transaction ID. APRS may later wrap a Core frame in a peer-scoped APRS
message ID and implement transport ACK, retries, duplicate suppression, result
replay, and fragmentation/reassembly. This PR does not implement that ledger.
`APRS ack<ID>` confirms packet reception; OpenQSP `STORED` confirms durable
application commit. Envelope IDs never become Message or Bulletin fields.
