...
SYNC_LIMIT = 20
BULLETIN_A = Bulletin(
    1296566273, 1786800001, SOURCE, "M4.8 bulletin A", "Complete body A"
)
BULLETIN_B = Bulletin(
    1296566274, 1786800002, SOURCE, "M4.8 bulletin B", "Complete body B"
)
BULLETIN_C = Bulletin(
    1296566275, 1786800003, SOURCE, "M4.8 bulletin C", "Complete body C"
)
MISSING_BULLETIN_ID = 1296629759
...
def _print_sync(since: int, responses: list[ProtocolObject]) -> None:
    ...
    for response in responses:
        if isinstance(response, BulletinHeader):
            ...
            print(
                f"END returned={response.returned_count} next_since={response.next_since} has_more={more}"
            )
