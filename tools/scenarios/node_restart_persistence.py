...
SYNC_LIMIT = 20
MESSAGE_A = SendMessage(0x4D340701, 1_786_700_001, RECIPIENT, "M4.7 message A")
MESSAGE_B = SendMessage(0x4D340702, 1_786_700_002, RECIPIENT, "M4.7 message B")
...
def main(argv: list[str] | None = None) -> int:
    ...
    _print(
        "Incremental synchronization returning only message B",
    ...
    print(
        "Sequence/cursor continuity: "
        f"sequence {first_message.sequence} -> {second_message.sequence}; "
        f"cursor {result.first_cursor} -> {_cursor(result.incremental_sync)}"
    )
