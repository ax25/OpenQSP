...
class TcpTransport:
    ...
    @staticmethod
    def _receive_exact(connection: socket.socket, size: int, *, header: bool) -> bytes:
        data = bytearray()
        while len(data) < size:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "seed-bulletin":
            ...
            bulletin = Bulletin(1, args.timestamp, args.callsign, args.title, args.body)
            ...
            print(f"  sequence: {outcome.sequence}")
